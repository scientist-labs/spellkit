...
RSpec.describe "Guards & Domain Policies (M2)" do
  ...
  describe "protected terms" do
    it "does not correct protected terms" do
      ...
      expect(SpellKit.correct("rat")).to eq("rat")
      expect(SpellKit.correct("mouse")).to eq("mouse")
    end

    it "handles IL-6 and IL6 variants" do
      expect(SpellKit.correct("IL6")).to eq("IL6")
      expect(SpellKit.correct("IL-6")).to eq("IL-6")
    end
  ...
  describe "protected patterns" do
    it "protects terms matching regex patterns" do
      ...
      expect(SpellKit.correct("7732-18-5")).to eq("7732-18-5")
  ...
  describe "regex flags" do
    ...
    it "respects case-sensitive patterns without flag" do
      ...
      expect(SpellKit.correct("il6")).to eq("il6")  # Protected now!
    end
    ...
    it "combines all three flags together (/imx)" do
      ...
      expect(SpellKit.correct("test")).to eq("test")
      expect(SpellKit.correct("TEST")).to eq("TEST")
      expect(SpellKit.correct("testing")).to eq("testing")
      expect(SpellKit.correct("TESTING")).to eq("TESTING")
      expect(SpellKit.correct("Testing")).to eq("Testing")
    end
    ...
    it "preserves pattern matching logic with case-insensitive flag" do
      ...
      expect(SpellKit.correct("BRCA1")).to eq("BRCA1")
      expect(SpellKit.correct("brca1")).to eq("brca1")
      expect(SpellKit.correct("TP538")).to eq("TP538")
      expect(SpellKit.correct("tp538")).to eq("tp538")

      # But NOT match different patterns
      result = SpellKit.correct("XYZ999")
  ...
  describe "normalized variant protection" do
    it "protects normalized variants of protected terms automatically" do
      ...
      expect(SpellKit.correct("New York")).to eq("New York")
      expect(SpellKit.correct("cell culture")).to eq("cell culture")
      ...
      expect(SpellKit.correct("newyork")).to eq("newyork")
      expect(SpellKit.correct("NewYork")).to eq("NewYork")
      expect(SpellKit.correct("cellculture")).to eq("cellculture")
      expect(SpellKit.correct("CellCulture")).to eq("CellCulture")
    ...
    it "protects terms with punctuation in all forms" do
      ...
      # Literal forms
      expect(SpellKit.correct("IL-6")).to eq("IL-6")
      expect(SpellKit.correct("p-value")).to eq("p-value")
    ...
    it "handles terms with mixed whitespace and punctuation" do
      ...
      # Literal forms
      expect(SpellKit.correct("New York, NY")).to eq("New York, NY")
      expect(SpellKit.correct("Smith, J.")).to eq("Smith, J.")

      # Normalized forms (whitespace stripped, punctuation preserved)
      expect(SpellKit.correct("newyork,ny")).to eq("newyork,ny")
      expect(SpellKit.correct("smith,j.")).to eq("smith,j.")
    ...
    it "handles multiple consecutive spaces in protected terms" do
      ...
      expect(SpellKit.correct("New  York")).to eq("New  York")
      expect(SpellKit.correct("test   term")).to eq("test   term")

      # Normalized forms (all spaces stripped) should also be protected
      expect(SpellKit.correct("newyork")).to eq("newyork")
      expect(SpellKit.correct("testterm")).to eq("testterm")
